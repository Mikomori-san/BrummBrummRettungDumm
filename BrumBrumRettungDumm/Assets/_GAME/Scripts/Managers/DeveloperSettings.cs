...
public class DeveloperSettings : MonoBehaviour
{
    ...
    public TrackingSphere trackingSphere;
    public Transform[] objectsToFollow;
    public float spawnRate;
    private float spawnTimer;
    public float lifeTime;
    [HideInInspector] public Queue<TrackingSphere> trackingSpheres = new Queue<TrackingSphere>();
    ...
    private void DrawSpheres()
    {
        ...
        if (this.spawnTimer <= 0)
        {
            ...
            {
                TrackingSphere tracker = this.trackingSpheres.Count > 0 ? this.trackingSpheres.Dequeue() : Instantiate(this.trackingSphere);

                tracker.lifeTime = this.lifeTime;
                tracker.transform.position = objectToFollow.position;
                tracker.gameObject.SetActive(true);
                tracker.sphereRenderer.material.color = tracker.normalColor;
            }
